...
app.post('/api/order', async (req,res)=>{
  try{
    const { package: pkg, wallet, customerName, customerEmail, txId, screenshot } = req.body;
    if(!customerName ||!customerEmail ||!screenshot) return res.status(400).json({error:'Missing fields'});
    const order = { _id: crypto.randomBytes(12).toString('hex'), package: pkg, wallet, customerName, customerEmail, txId, screenshot, status: 'pending', createdAt: new Date() };
